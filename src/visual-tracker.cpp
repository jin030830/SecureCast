// =============================================================================
// visual-tracker.cpp — NCC 기반 블러 박스 위치 추적기 (구현부)
//
// 왜 필요한가:
//   OCR(ocr-engine)은 "무엇이 개인정보인가"를 알아내지만 느리다(~250ms, 초당
//   4회 수준). 그 사이 창이 움직이거나 스크롤되면 OCR이 알려준 좌표는 금방
//   낡아 블러가 글자에서 어긋난다. 이 추적기는 OCR이 한 번 찾아준 박스를
//   매 프레임(~16ms) 따라다니게 해서 "어디로 움직였는가"를 메운다.
//   즉 역할 분담: OCR = what(느림) / Tracker = where(빠름).
//
// 핵심 동작:
//   · register_or_update(): OCR이 새 박스를 알려주면, 그 박스 영역의 회색조
//     픽셀을 "템플릿"으로 저장하고 IoU로 기존 추적기와 매칭/재동기화한다.
//   · update_all_gray(): 매 렌더 프레임마다 각 템플릿을 현재 화면에서 NCC
//     (정규화 상호상관)로 가장 닮은 위치를 찾아 박스 좌표를 갱신한다.
//     - 2단계 피라미드 검색(1/4 축소본에서 대략 → 원본에서 정밀)
//     - AVX2/SSSE3 SIMD로 가속, 미지원 CPU는 스칼라 폴백
//     - 점수가 SCORE_LOST 미만이 연속되면 "놓침"으로 판정해 박스 제거
//   · snapshot_for_push(): 창에 바인딩(anchor)된 박스는 NCC 대신 창의 현재
//     DWM 위치 변화량(delta)만큼 즉시 평행이동 → 추적 지연 없이 창과 함께 이동.
//   · active_boxes(): 렌더 스레드가 현재 블러를 그릴 좌표를 조회.
//
// 부가 기능:
//   · 스크롤/키 입력(scroll_motion_hook) 신호를 받아, 화면이 흐르는 동안 박스를
//     위아래로 확장(hysteresis)해 스크롤 중 순간 노출을 막는다.
//
// 스레드 안전:
//   내부 shared_mutex로 보호 — active_boxes()는 공유 락(읽기), 등록/갱신은
//   배타 락(쓰기). 렌더 스레드와 OCR 워커가 동시에 안전하게 호출한다.
// =============================================================================
