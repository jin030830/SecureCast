...
// Adaptive OCR rate (video_tick에서 ocrRateInterval_에 반영)
//   회귀 시: OCR_RATE_CPU_MED/LOW를 100.0f로 올려 사실상 비활성
static constexpr float OCR_RATE_DEFAULT_S = 0.25f; // 4fps  (CPU < MED)
static constexpr float OCR_RATE_MEDIUM_S  = 0.5f;  // 2fps  (CPU ≥ MED)
static constexpr float OCR_RATE_LOW_S     = 1.0f;  // 1fps  (CPU ≥ LOW)
static constexpr float OCR_RATE_CPU_MED   = 20.0f; // 중간 임계 (%)
static constexpr float OCR_RATE_CPU_LOW   = 30.0f; // 저속 임계 (%)
// Window pre-filter: 이 소스 ID면 OCR 완전 스킵
//   롤백: nullptr 또는 빈 문자열로 교체 → pre-filter 비활성
static constexpr const char *OCR_SKIP_SOURCE_ID = "game_capture";
// ────────────────────────────────────────────────────────────────
...
static void securecast_video_render(void *data, gs_effect_t *effect) {
  ...
  if (filter->trackerFrameSkip_ >= 2) {
    ...
    const bool isGameCaptureSource =
        parentSrcId && OCR_SKIP_SOURCE_ID &&
        strcmp(parentSrcId, OCR_SKIP_SOURCE_ID) == 0;
...
static void securecast_video_tick(void *data, float seconds) {
  ...
  if (filter->cpuSampleAccumulator >= GM_SAMPLE_INTERVAL) {
    ...
    if (!filter->isGameMode) {
      if (filter->cpuUsage >= OCR_RATE_CPU_LOW)
        filter->ocrRateInterval_ = OCR_RATE_LOW_S;
      else if (filter->cpuUsage >= OCR_RATE_CPU_MED)
        filter->ocrRateInterval_ = OCR_RATE_MEDIUM_S;
      else
        filter->ocrRateInterval_ = OCR_RATE_DEFAULT_S;
    }
  }
