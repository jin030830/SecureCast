...
// 도착할 때마다 atomic needRescan 플래그를 set. 다음 video_tick 에서 그
// 플래그를 보고 풀 스캔 throttle 을 무시하고 즉시 처리.
//
// 효과:
//   - 카톡 등장/사라짐/위치변경 평균 latency 50ms → ~16ms (1 video_tick)
//   - 풀 스캔 throttle 을 더 길게 (예: 500ms) 잡아도 push 로 즉시 반응 → CPU
//   절감
//
// 안전성:
...
public:
  ...
  // video_tick 에서 호출. 이벤트가 한 번이라도 있었으면 true 를 반환하면서 flag
  // 리셋.
...
private:
  ...
  HWINEVENTHOOK m_hookGroup3 =
      nullptr; // SYSTEM_MINIMIZESTART / MINIMIZEEND (최소화/복원)

  std::atomic<bool> m_needRescan{false};

  // 콜백에서 도달하기 위한 스레드별 self 포인터 (한 프로세스에 instance 1개
  // 가정).
