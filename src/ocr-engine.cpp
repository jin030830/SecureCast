...
struct SecureCastOcrEngine::Impl {
  ...
  // stop_ocr_worker가 RecognizeAsync 블로킹 중에 Cancel()을 보낼 수 있도록
  // 보관. opMutex로 worker/main 스레드 간 접근을 보호한다.
...
SecureCastOcrEngine::SecureCastOcrEngine() : impl_(std::make_unique<Impl>()) {}
...
bool SecureCastOcrEngine::available() const { return available_; }
...
SecureCastOcrEngine::analyze_bgra_frame(const uint8_t *pixels, int width,
  ...
  // 변경된 라인만 crop OCR → 전체 라인 텍스트 합산 후 detect_pii 1회 실행.
  // prevHadName 분기 제거: 캐시된 text로 인접줄 컨텍스트를 보존하므로 NAME도 L2
  // 가능.
  if (sameRes && !lastLineDhashes_.empty()) {
    std::vector<SecureCastOcrLine> allLines;
    bool anyChanged = false;

    for (auto &lc : lastLineDhashes_) {
      if (lc.x < 0 || lc.y < 0 || lc.x + lc.w > width || lc.y + lc.h > height)
        continue;
      ...
      if (hamming_distance(lineHash, lc.dhash) <= 2) {
        // 불변 라인: 캐시된 텍스트로 OcrLine 재구성 (인접줄 컨텍스트 보존)
        if (!lc.text.empty()) {
          SecureCastOcrLine cached;
          cached.text = lc.text;
          cached.x = (float)lc.x;
          cached.y = (float)lc.y;
          cached.w = (float)lc.w;
          cached.h = (float)lc.h;
          allLines.push_back(std::move(cached));
        }
      } else {
        anyChanged = true;
        // 변경된 라인만 crop OCR
        auto cropLines = recognize_text_crop(pixels, width, height, stride,
                                             lc.x, lc.y, lc.w, lc.h);
        // L2 캐시 갱신 (텍스트 + dhash)
        lc.dhash = lineHash;
        lc.text.clear();
        for (const auto &cl : cropLines) {
          if (!lc.text.empty())
            lc.text += ' ';
          lc.text += cl.text;
          allLines.push_back(cl);
        }
        lc.hasBox = false;
        lc.box = {};
      }
    ...
    if (anyChanged) {
      // detect_pii 1회 — NAME + 인접 라인 컨텍스트 전부 전달
      auto merged = detect_pii(allLines);
      // 변경된 캐시 라인의 hasBox/box 갱신
      for (auto &lc : lastLineDhashes_) {
        lc.hasBox = false;
        lc.box = {};
  ...
  for (const auto &line : updatedLines) {
    ...
    for (const auto &b : boxes) {
      if ((int)b.x >= lc.x && (int)b.y >= lc.y &&
          (int)(b.x + b.w) <= lc.x + lc.w && (int)(b.y + b.h) <= lc.y + lc.h) {
        lc.hasBox = true;
        lc.box = b;
        break;
      }
...
SecureCastOcrEngine::recognize_text_crop(const uint8_t *px, int width,
  ...
  cx = std::max(0, cx);
  cy = std::max(0, cy);
  cw = std::min(cw, width - cx);
  ch = std::min(ch, height - cy);
  if (!px || cw <= 0 || ch <= 0)
    return {};
...
std::vector<SecureCastOcrLine>
SecureCastOcrEngine::recognize_text(const uint8_t *pixels, int width,
                                    int height, int stride) {
  std::vector<SecureCastOcrLine> lines;

  if (!impl_->engine || pixels == nullptr || width <= 0 || height <= 0 ||
      stride <= 0) {
    return lines;
  }
...
static bool decode_utf8_next(const std::string &text, size_t &index,
                             uint32_t &codepoint, size_t &charStart,
                             size_t &charLength) {
  if (index >= text.size()) {
    return false;
  }
  ...
  if ((c0 & 0xF8) == 0xF0 && index + 3 < text.size()) {
    ...
    codepoint = ((c0 & 0x07) << 18) | ((c1 & 0x3F) << 12) | ((c2 & 0x3F) << 6) |
                (c3 & 0x3F);
    charLength = 4;
    index += 4;
    return true;
...
static bool has_adjacent_name_label(const std::vector<SecureCastOcrLine> &lines,
                                    int index) {
...
static bool valid_card_iin(const std::string &digits) {
  ...
  if (p1 == 4)
    return true; // Visa
  if (p2 >= 51 && p2 <= 55)
    return true; // Mastercard (classic)
  if (p4 >= 2221 && p4 <= 2720)
...
SecureCastOcrEngine::detect_pii(const std::vector<SecureCastOcrLine> &lines) {
  ...
  static const re2::RE2 PATTERN_NAME_HONORIFIC(R"(([가-힣]{2,4})\s*(?:님|씨))");
  ...
  for (int i = 0; i < static_cast<int>(lines.size()); ++i) {
    ...
    // OCR이 "user" / "@gmail.com" 처럼 @ 앞뒤를 다른 줄로 쪼개는 경우 결합 후
    // 재검사.
    if (type == nullptr) {
    ...
    if (type == nullptr && rawText.find('@') != std::string::npos) {
      static const auto has_email_label = [](const std::string &t) -> bool {
        return contains_any(t, {"이메일", "메일", "Email", "EMAIL", "E-mail",
    ...
    if (type == nullptr && i + 1 < static_cast<int>(lines.size())) {
      if (contains_any(
        ...
        if (looks_like_korean_address_text(combined)) {
          ...
          boxes.push_back(SecureCastOcrBox{"ADDRESS", lines[i + 1].x,
                                           lines[i + 1].y, lines[i + 1].w,
                                           lines[i + 1].h});
          ++i;
          continue;
        }
    ...
    if (type == nullptr) {
      ...
      if (type == nullptr &&
        ...
        auto hasLabel = [](const std::string &t) {
          // 정상 라벨
          if (t.find("\xEA\xB3\x84\xEC\xA2\x8C") != std::string::npos)
            return true; // 계좌
          if (t.find("\xEC\x98\x88\xEA\xB8\x88\xEC\xA3\xBC") !=
              std::string::npos)
            return true; // 예금주
          if (t.find("\xEC\x9E\x85\xEA\xB8\x88") != std::string::npos)
            return true; // 입금
          if (t.find("\xEC\x86\xA1\xEA\xB8\x88") != std::string::npos)
            return true; // 송금
          if (t.find("\xEC\x9D\x80\xED\x96\x89") != std::string::npos)
            return true; // 은행
          if (t.find("Account") != std::string::npos)
            return true;
          if (t.find("ACCT") != std::string::npos)
            return true;
          // OCR 오인식 변형
          if (t.find("\xEA\xB2\x8C\xEC\xA2\x8C") != std::string::npos)
            return true; // 게좌
          if (t.find("\xEA\xB3\x84\xEC\x9E\x90") != std::string::npos)
            return true; // 계자
          if (t.find("\xEA\xB3\x84\xEC\xA1\xB0") != std::string::npos)
            return true; // 계조
          return false;
        };
        ...
        for (int d = 1; d <= 2 && !hasAccountLabel; ++d) {
          ...
          if (i + d < static_cast<int>(lines.size()) &&
              hasLabel(lines[i + d].text))
            hasAccountLabel = true;
        }
        if (hasAccountLabel)
          type = "ACCOUNT";
    ...
    if (type == nullptr) {
      ...
      if (sameLineLabel || adjLabel) {
        ...
        const std::string hangulOnly =
            extract_hangul_syllables_utf8(candidateText);
    ...
    if (type != nullptr) {
      boxes.push_back(SecureCastOcrBox{type, line.x, line.y, line.w, line.h});
    }
  }
